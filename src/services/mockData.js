...
import { Normal_status_map, Alarm_status_map } from './detailMap';

// SRM values
const SRM_VALUES = [1, 2, 3, 4, 5, 6, 7, 8];

// PLC code values - using keys from the mapping files
const NORMAL_PLCCODE_VALUES = Object.keys(Normal_status_map).map(Number);
const ALARM_PLCCODE_VALUES = Object.keys(Alarm_status_map).map(Number);
...
export function generateMockData(numRecords = 200, startDate, endDate) {
  ...
  for (let i = 0; i < numRecords; i++) {
    ...
    if (isNormal) {
      ...
      msglog = Normal_status_map[plccode] || "Unknown normal status";
      msgtype = "Normal";
      normalCount++;
    } else {
      plccode = getRandomItem(ALARM_PLCCODE_VALUES);
      msglog = Alarm_status_map[plccode] || "Unknown alarm status";
      msgtype = "Alarm";
      alarmCount++;
    }
